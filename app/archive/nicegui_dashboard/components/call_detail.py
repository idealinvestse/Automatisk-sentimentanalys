...
from app.archive.nicegui_dashboard.components.call_alerts_section import render_call_alerts_section
...
from app.archive.nicegui_dashboard.components.evidence_panel import render_evidence_panel
from app.archive.nicegui_dashboard.components.llm_judge_panel import render_llm_judge_panel
from app.archive.nicegui_dashboard.components.pii_audit import render_pii_audit_panel
from app.archive.nicegui_dashboard.components.ui_primitives import render_section_title
from app.archive.nicegui_dashboard.components.virtual_transcript import (
    render_timeline,
...
from app.archive.nicegui_dashboard.services.transcript_virtualizer import filter_segments_with_index
...
def _build_insights_markdown(report: dict[str, Any]) -> str:
    ...
    if isinstance(assess, dict) and assess:
        ...
        if strengths:
            parts.append(f"**Styrkor:** {', '.join(html.escape(str(s)) for s in strengths[:4])}")
        coaching = assess.get("specific_coaching_recommendations") or []
        if coaching:
            parts.append("**Coaching:**")
            for c in coaching[:3]:
                parts.append(f"- {html.escape(str(c))}")
