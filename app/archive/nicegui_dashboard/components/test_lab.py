...
from app.archive.nicegui_dashboard.components.live_analysis import render_text_pipeline_section
            ...
            try:
                if action == "asr":
                    ...
                    res = await asyncio.to_thread(
                        ...
                        language=settings["language"],
                        expected_sentiment=(
                            row.get("expected") if row.get("expected") != "—" else None
                        ),
                    )
        ...
        for row in targets:
            ...
            try:
                if action == "asr":
                    ...
                    out_rows.append(
                        {"path": row["path"], "ok": "✓" if text else "✗", "detail": text[:120]}
                    )
                elif action == "pipeline":
    ...
    async def run_scenario() -> None:
        ...
        try:
            ...
            _render_results_panel(
                results_container, report, title=f"Scenario: {scenario_select.value}"
            )
            notify_success("Scenario klart")
        except Exception as exc:
            notify_error(str(exc))

    ui.button("Kör scenario", color="primary", icon="play_arrow", on_click=run_scenario).classes(
        "q-mt-sm"
    )
...
def _render_text_section(state: DashboardState) -> None:
    ...
    custom_text = ui.textarea(
        label="Eller egen text", placeholder="Skriv text att analysera..."
    ).classes("w-full")
            ...
            with text_result:
                if results:
                    ...
                    if isinstance(first, list) and first:
                        ...
                        ui.label(
                            f"Label: {best.get('label')} (score {best.get('score', 0):.2f})"
                        ).classes("text-body1")
                    else:
    ...
    ui.button("Analysera text", icon="sentiment_satisfied", on_click=run_text_sentiment).classes(
        "q-mt-sm"
    )
...
def _render_system_section(state: DashboardState) -> None:
    ...
    if media_root:
        ...
        ui.label(f"API_MEDIA_ROOT ej satt — rekommenderat: {repo_root()}").classes(
    ...
    async def run_doctor() -> None:
        ...
        try:
            ...
            with checks_container:
                ...
                ui.label(
                    f"{icon} {'Alla kontroller OK' if report.ok else 'Vissa kontroller misslyckades'}"
