...
def compute_kpis(
    ...
    len(filtered) if filtered else 1
    ...
    for r in filtered:
        ...
        if topics:
            hot_count += (
                len(topics.get("topics", [])) if isinstance(topics.get("topics"), list) else 1
            )
        llm = r.get("llm") or {}
    ...
    risky = sum(
        1
        for r in filtered
        if str(((r.get("results") or {}).get("qa") or {}).get("risk_level", "")).lower()
        in {"high", "critical"}
    ...
    return {
        ...
        "risky_calls": risky,
        "avg_processing_s": round(
            sum(r.get("processing_time_s", 0) for r in filtered) / max(1, total), 2
        ),
    }
...
def get_agent_leaderboard(reports: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    by_agent: dict[str, dict] = defaultdict(
        lambda: {"calls": 0, "empathy_sum": 0.0, "qa_sum": 0.0, "qa_n": 0, "coaching": 0}
    )
    for r in reports:
        ...
        if isinstance(assess, dict) and assess.get("empathy_score") is not None:
            by_agent[agent]["empathy_sum"] += float(
                assess["empathy_score"]
            )  # double weight if both
