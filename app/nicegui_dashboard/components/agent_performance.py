...
def render_agent_performance_tab(
    ...
    *,
    ...
    def performance_section() -> None:
        ...
        source_lbl = ui.label("").classes("text-caption")
        ...
        metrics_holder: dict[str, Any] = {"data": local_agent_metrics(agent_id, reports)}

        def _on_agent_change(e: Any) -> None:
        ...
        with ui.row().classes("w-full gap-4 flex-wrap items-end q-mb-md"):
            ...
            ui.button(
                icon="refresh",
                on_click=lambda: _refresh_metrics(agent_id, reports, metrics_holder, source_lbl),
        ...
        def _render_metrics(metrics: dict[str, Any]) -> None:
            ...
            source_lbl.set_text(f"Metrics: {metrics_source['value']}")
        ...
        async def _refresh_metrics(
            ...
            if loading["active"]:
                return
            loading["active"] = True
            lbl.set_text("Laddar metrics...")
            try:
                holder["data"] = await _load_metrics(aid, reps)
            except Exception as err:
                notify_api_error(err)
                holder["data"] = local_agent_metrics(aid, reps)
            finally:
                loading["active"] = False
            _render_metrics(holder["data"])

        _render_metrics(metrics_holder["data"])
        ui.timer(0.05, lambda: _refresh_metrics(agent_id, reports, metrics_holder, source_lbl), once=True)
