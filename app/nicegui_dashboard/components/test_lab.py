...
def create_llm_model_settings() -> None:
    ...
    with ui.card().classes('w-full'):
        ...
        ui.label('Scanna alla tillgängliga modeller + hämta live-pricing och info').classes('text-sm text-gray-600')
        ...
        def refresh_catalog():
            ...
            try:
                ...
                ui.notify('Model catalog uppdaterad! openrouter_client använder nu dynamisk pricing.', type='positive')
        ...
        def show_top_models():
            ...
            with open(p) as f:
                cat = json.load(f)
            models = sorted(cat['models'], key=lambda m: m['pricing']['prompt_per_million_usd'] or 999)[:5]
            with ui.dialog() as dlg, ui.card():
                ui.label('Top 5 billigaste modeller').classes('text-lg')
                for m in models:
                    p = m['pricing']
                    ui.label(f"{m['id']} | Prompt: ${p['prompt_per_million_usd']:.4f}/M | Completion: ${p['completion_per_million_usd']:.4f}/M").classes('text-sm')
...
try:
    ...
    pass  # Om körs utan NiceGUI context
