...
## 7. Recommended Next Development Tasks (as of 2026-06-26)
**Generated by github-repo-deep-dive skill after user clarifying questions.**

Användarens huvudprioritet: **Polerad Fas 4-dashboard som QA-team använder dagligen** + foundational improvements + bättre UX/reliability.

**Top recommended tasks** (ranked):

**TASK-01 (Högst prioritet)**: Wire LLMJudgeAnalyzer fullt in i CallAnalysisPipeline + API  
*Why now*: LLM-judge är implementerat men inte wired. Detta är foundational och unblockar all dashboard-visualisering av judge-verdicts. Högst värde för QA-team.

**TASK-02**: Skapa/utöka NiceGUI-komponent för LLM-judge verdicts + integrera i Fas 4-dashboard (call_detail / fas4_insights)  
*Why now*: Direkt adresserar "polerad dashboard som används dagligen". Ger synlig nytta.

**TASK-03**: Förbättra caching + async data fetching i NiceGUI dashboard (foundational reliability/UX)  
*Why now*: Gör dashboarden snabb och stabil för daglig användning. Stödjer både TASK-01/02 och långsiktigt reliability-mål.

**TASK-04**: Polera Alerting webhook + enkel test-UI i test_lab  
*Why now*: En av de delar som nämndes som smärtsam. Redan delvis implementerat (circuit breaker), behöver bara polish.

Se full detalj i `RECOMMENDED_NEXT_TASKS.md` (skapad samma dag).
...
- Always start new session by reading: `docs/LLM_AGENT_GUIDE.md`, `PROJECT_STATUS.md`, `RECOMMENDED_NEXT_TASKS.md`, `ROADMAP.md`, `SECURITY.md`.
