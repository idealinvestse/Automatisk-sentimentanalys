...
- If a test fails, check whether it's a genuine logic regression or an **environment/dependency gap** before assuming the code is broken — e.g. a missing optional dependency can make a real ML model fail to load and silently trigger a fallback path, which looks like a business-logic assertion failure rather than an import error. A worked example is missing `sentencepiece` masquerading as a sentiment/intent regression in `tests/test_callcenter_golden.py` (install `[dev]` extras).
