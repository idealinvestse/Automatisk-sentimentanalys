...
### Fas 0: Setup & Baseline (0,5 dag)
**Tasks:**
1. Skapa branch `api-review-v2-hardening` från main.
2. Kör full linter-suite: `ruff check --fix`, `mypy src/api/`, `bandit -r src/api/`.
3. Generera baseline OpenAPI och spara som `reports/api_openapi_baseline.json`.
4. Kör befintliga API-tester med coverage: `pytest tests/test_api.py tests/test_pipeline.py -q --cov=src/api --cov-report=html:reports/coverage_html`.
5. Inventera alla endpoints i en tabell (path, method, request model, response model, auth, cache, notes).

**Deliverables:** Branch skapad, baseline-rapporter, inventory-tabell i denna plan (uppdateras).

### Fas 1: Djupgående Kodgranskning & Bugghunt (1 dag)
...
- deep_analysis copy-paste i `src/api/routers/pipeline.py` (nu explicit + getattr fallback).
